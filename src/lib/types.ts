...
export const ExperienceFieldsSchema = z.object({
  ...
  weight_signals: z.object({
    ...
    repeated: WeightSignal.describe("② 방식의 반복: 이번 경험에서 본인이 정한 방식이 다른 대상에서도 다시 나왔다(자소서 → 보고서 같은 같은 계열도 인정, 자소서 → 영상 같은 먼 대상도 인정). 같은 종류의 것에서 다시 한 것(다른 자소서를 같은 방식으로 또 씀)과 하고 싶다는 의향만 제외"),
