...
import { jobWorkSummary } from "../src/lib/jobfinder";
...
async function build(id: string) {
  ...
  md.push(
    "## 2. 검토 메모 (참가자에게 보내지 않음)\n\n" +
      [
        "### 코어 행동 ← 코어 판정의 행동 설명",
        ...(s.report?.cores ?? []).map((c, i) => {
          const b = s.final?.cores[i]?.behavior;
          return `- '${coreBehavior(c)}' ← ${b ? `${b.action} / ${b.shape} / ${b.criterion || "기준 없음"}` : ((c as { core?: string }).core ?? "")}`;
        }),
        "",
        "### 코어 판정",
